...
export async function generateStaticParams() {
  return getComparableMessageParams(MessageSource.Roadmap)
}
...
export default async function ComparePage({ params }: Props) {
  ...
  // Keep the source split clean: only roadmap-sourced records render here.
  const latest = history.versions[history.versions.length - 1]?.message ?? message
  if (getMessageSource(latest) !== MessageSource.Roadmap) notFound()
